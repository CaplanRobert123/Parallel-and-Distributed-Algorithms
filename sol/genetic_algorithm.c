...
void compute_fitness_function(const sack_object *objects, individual *generation, int object_count, int sack_capacity, int id, int P, pthread_barrier_t *barrier)
{
	...
	// printf("thread id from compute fitness: %d \n", id);
	// printf("barrier from fitness: %p \n", barrier);
	...
	for (int i = start; i < end; ++i)
	{
		weight = 0;
		profit = 0;
		generation[i].updated_chromosomes = 0;
		...
		for (int j = 0; j < generation[i].chromosome_length; ++j)
		{
			...
			if (generation[i].chromosomes[j])
			{
				...
				generation[i].updated_chromosomes += generation[i].chromosomes[j];
			}
		}
...
int cmpfunc(const void *a, const void *b)
{
	...
	if (res == 0)
	{
		...
		res = first->updated_chromosomes - second->updated_chromosomes; // increasing by number of objects in the sack
		if (res == 0)
		{
